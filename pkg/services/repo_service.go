...
func NewArgoCDService(db db.ArgoDB, repoServerAddress string, credsStore git.CredsStore) Repos {
...
func (a *argoCDService) GetFiles(ctx context.Context, repoURL string, revision string, pattern string) (map[string][]byte, error) {
	...
	gitRepoClient, err := git.NewClient(repo.Repo, repo.GetGitCreds(a.credsStore), repo.IsInsecure(), repo.IsLFSEnabled(), repo.Proxy)
